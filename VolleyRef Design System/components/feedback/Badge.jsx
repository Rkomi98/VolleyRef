...
export function Badge({ children, tone = 'neutral', size = 'md', icon = null }) {
  const t = tones[tone] || tones.neutral;
  const pad = size === 'sm' ? '2px 8px' : '4px 10px';
  const fontSize = size === 'sm' ? 'var(--text-xs)' : 'var(--text-sm)';
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: pad, borderRadius: 'var(--radius-full)',
      background: t.bg, color: t.fg, fontSize, fontFamily: 'var(--font-body)', fontWeight: 'var(--weight-semibold)', lineHeight: 1.4 }}>
      {icon}{children}
    </span>
  );
}
