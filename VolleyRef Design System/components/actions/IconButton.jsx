...
export function IconButton({ icon, label, size = 'md', variant = 'default', active = false, disabled = false, onClick, style }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 'var(--radius-sm)', border: 'none', background: bg, color: fg,
        cursor: disabled ? 'not-allowed' : 'pointer', transition: 'background var(--duration-fast) var(--ease-standard)',
        ...style,
      }}
    >
      {icon}
