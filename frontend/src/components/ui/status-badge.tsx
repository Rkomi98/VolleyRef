...
const AlertIcon = (p: IconProps) => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" {...p}>
    <path
...
const ErrorIcon = (p: IconProps) => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8" {...p}>
    <circle cx="12" cy="12" r="9" />
    <path d="M9.5 9.5l5 5m0-5l-5 5" strokeLinecap="round" />
  </svg>
)
const DotIcon = (p: IconProps) => (
...
const MAP: Record<
  "validated" | "review" | "inconsistent" | "processing",
  { label: string; bg: string; fg: string; Icon: (p: IconProps) => React.JSX.Element }
> = {
...
export function StatusBadge({ status = "validated", children, size = "md" }: StatusBadgeProps) {
